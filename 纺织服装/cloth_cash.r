...
cash <- report_clean(cloth$corp_name, cloth$report_period, cloth$经营活动现金净流量)
cash[cash == 0] <- NA
...
mean(abs(m$model$residuals/dat_cloth$cash))
summary(m$model)
vif(m$model)

df <- data.frame(time=t,
                 actual=dat_cloth$cash,
...
index <- m$index[-c(2,3,8,9,10)]
lm1 <- lm(cash~., data = dat_cloth[,c(0,index)+1])
mean(abs(lm1$residuals/dat_cloth$cash))
summary(lm1)
vif(lm1)
...
# df <- melt(df, id='time')
# ggplot(df, aes(time, value, color=variable)) + geom_line()
# write.csv(df, file = '纺织服装行业现金流.csv')
# sink('纺织服装行业现金流模型.txt')
# summary(lm1)
# lm1$coefficients
# print('置信区间:')
# pred$fit
