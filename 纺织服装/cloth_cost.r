...
cost <- report_clean(cloth$corp_name, cloth$report_period, cloth$营业总成本)
cost[cost == 0] <- NA
...
mean(abs(m$model$residuals/dat_cloth$cost))
summary(m$model)
vif(m$model)

df <- data.frame(time=t,
                 actual=dat_cloth$cost,
...
index <- m$index[-c(6,9)]
lm1 <- lm(cost~., data = dat_cloth[,c(0,index)+1])
mean(abs(lm1$residuals/dat_cloth$cost))
summary(lm1)
vif(lm1)
...
# df <- melt(df, id='time')
# ggplot(df, aes(time, value, color=variable)) + geom_line()
# write.csv(df, file = '纺织服装行业成本.csv')
# sink('纺织服装行业成本模型.txt')
# summary(lm1)
# lm1$coefficients
# print('置信区间:')
