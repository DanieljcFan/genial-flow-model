...
revenue <- report_clean(cloth$corp_name, cloth$report_period, cloth$营业总收入)
...
m <- Model(dat_cloth$revenue,
           dat_cloth[-1],
           vif=10,
           method = 'aic'
)

mean(abs(m$model$residuals/dat_cloth$revenue))
summary(m$model)
vif(m$model)

df <- data.frame(time=t,
                 actual=dat_cloth$revenue,
...
index <- m$index[-c(1,9,13)]
lm1 <- lm(revenue~., data = dat_cloth[,c(0,index)+1])
mean(abs(lm1$residuals/dat_cloth$revenue))
summary(lm1)
vif(lm1)
...
# df <- melt(df, id='time')
# ggplot(df, aes(time, value, color=variable)) + geom_line()
# write.csv(df, file = '纺织服装行业营收.csv')
# sink('纺织服装行业营收模型.txt')
# summary(lm1)
# lm1$coefficients
# print('置信区间:')
