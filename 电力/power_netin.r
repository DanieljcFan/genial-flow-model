...
netin <- report_clean(power$corp_name, power$report_period, power$净利润)
netin[netin == 0] <- NA
...
#合并X,Y，时间戳另存为t
dat_power <- merge(netin,X_all)
t <- dat_power[,1] 
dat_power <- dat_power[,-1]

#model#####
m <- Model(dat_power$netin,
           dat_power[-1],
           vif=5,
           must = 12,
           method = 'aic')

mean(abs(m$model$residuals/dat_power$netin)) #相对误差
summary(m$model) 
vif(m$model)

df <- data.frame(time=t,
                 actual=dat_power$netin,
...
#手动剔除变量，修改模型
index <- m$index[-c(2,6)]
lm1 <- lm(netin~., data = dat_power[,c(0,index)+1])
mean(abs(lm1$residuals/dat_power$netin))
summary(lm1)
vif(lm1)
