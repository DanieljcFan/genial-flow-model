...
cost <- report_clean(rubber$corp_name, rubber$report_period, rubber$营业总成本)
cost[cost == 0] <- NA
...
mean(abs(m$model$residuals/dat_rubber$cost))
summary(m$model)
vif(m$model)

df <- data.frame(time=t,
                 actual=dat_rubber$cost,
...
index <- m$index[-c(3,4,5,6,8)]
lm1 <- lm(cost~., data = dat_rubber[,c(0,index)+1])
mean(abs(lm1$residuals/dat_rubber$cost))
summary(lm1)
vif(lm1)
