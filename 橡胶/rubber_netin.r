...
netin <- report_clean(rubber$corp_name, rubber$report_period, rubber$净利润)
netin[netin == 0] <- NA
...
mean(abs(m$model$residuals/dat_rubber$netin))
summary(m$model)
vif(m$model)

df <- data.frame(time=t,
                 actual=dat_rubber$netin,
...
index <- m$index[-c(3,4,7,8,9)]
lm1 <- lm(netin~., data = dat_rubber[,c(0,index)+1])
mean(abs(lm1$residuals/dat_rubber$netin))
summary(lm1)
vif(lm1)
