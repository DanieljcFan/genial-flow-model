...
cash <- report_clean(rubber$corp_name, rubber$report_period, rubber$归属母公司股东的净利润)
cash[cash == 0] <- NA
...
mean(abs(m$model$residuals/dat_rubber$cash))
summary(m$model)
vif(m$model)

df <- data.frame(time=t,
                 actual=dat_rubber$cash,
...
index <- m$index[-c(2,7,8,9)]
lm1 <- lm(cash~., data = dat_rubber[,c(0,index)+1])
mean(abs(lm1$residuals/dat_rubber$cash))
summary(lm1)
vif(lm1)
