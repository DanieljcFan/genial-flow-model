...
revenue <- report_clean(rubber$corp_name, rubber$report_period, rubber$营业总收入)
...
#读入数据，需对Excel表做预处理
#注意时间列的格式（‘/’分割 or ‘-’分割）
setwd("I:/work/genial-flow/橡胶/")
chain <- read.csv('橡胶.csv')
names(chain)[1] <- 'time'
...
# #各省产量加总
# index <- grep('产量.橡胶', names(chain_season))
# chain_season$橡胶产量 <- rowSums(chain_season[,index])
# chain_season <- chain_season[,-index]
# 

# #补充变量
# tmp <- read.csv('橡胶营收.csv')
# names(tmp)[1] <- 'time'
...
#剔除避孕套进口量，无法解释
chain_season <- chain_season[,-grep('避孕套',names(chain_season))]
...
dat_rubber <- merge(revenue,X_all)
t <- dat_rubber[,1]
dat_rubber <- dat_rubber[,-1]
...
m <- Model(dat_rubber$revenue,
           dat_rubber[-1],
           vif=5,
           must = ncol(dat_rubber)-1,
           method = 'aic'
)

mean(abs(m$model$residuals/dat_rubber$revenue))
summary(m$model)
vif(m$model)

df <- data.frame(time=t,
                 actual=dat_rubber$revenue,
                 pred=m$model$fitted.values)
df <- melt(df, id='time')
ggplot(df, aes(time, value, color=variable)) + geom_line()

# tmp <- dat_rubber[,m$index+1]
# names(tmp) <- ''
# corr <- cor(tmp)
# corrplot.mixed(corr, diag = 'n', tl.pos = 'lt')

index <- m$index[-c(3,5,6,8)]
lm1 <- lm(revenue~., data = dat_rubber[,c(0,index)+1])
mean(abs(lm1$residuals/dat_rubber$revenue))
summary(lm1)
vif(lm1)
