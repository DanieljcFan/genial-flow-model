...
cash <- report_clean(steel$corp_name, steel$report_period, steel$经营活动现金净流量)
...
mean(abs(m$model$residuals/dat_steel$cash))
summary(m$model)
vif(m$model)
df <- data.frame(time=t,
                 actual=dat_steel$cash,
...
index <- m$index[-c(7:10)]
lm1 <- lm(cash~., data = dat_steel[,c(0,index)+1])
mean(abs(lm1$residuals/dat_steel$cash))
summary(lm1)
vif(lm1)
