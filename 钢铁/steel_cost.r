...
cost <- report_clean(steel$corp_name, steel$report_period, steel$营业总成本)
...
dat_steel <- merge(cost, X, by='time')
t <- dat_steel[,1]
...
mean(abs(m$model$residuals/dat_steel$cost))
summary(m$model)
vif(m$model)
