...
cost <- report_clean(engine$corp_name, engine$report_period, engine$营业总收入)
cost[cost == 0] <- NA
...
mean(abs(m$model$residuals/dat_engine$cost))
summary(m$model)
vif(m$model)

df <- data.frame(time=t,
                 actual=dat_engine$cost,
                 pred=m$model$fitted.values)
df <- melt(df, id='time')
ggplot(df, aes(time, value, color=variable)) + geom_line()

# tmp <- dat_engine[,m$index+1]
# names(tmp) <- ''
# corr <- cor(tmp)
# corrplot.mixed(corr, diag = 'n', tl.pos = 'lt')

index <- m$index[-c(2)]
lm1 <- lm(cost~., data = dat_engine[,c(0,index)+1])
mean(abs(lm1$residuals/dat_engine$cost))
summary(lm1)
vif(lm1)
