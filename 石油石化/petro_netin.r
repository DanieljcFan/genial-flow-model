...
netin <- report_clean(petro$corp_name, petro$report_period,petro$净利润)
netin[netin == 0] <- NA
...
mean(abs(m$model$residuals/dat_petro$netin))
summary(m$model)
vif(m$model)

df <- data.frame(time=t,
                 actual=dat_petro$netin,
...
index <- m$index[-c(3,5,7)]
lm1 <- lm(netin~., data = dat_petro[,c(0,index)+1])
mean(abs(lm1$residuals/dat_petro$netin))
summary(lm1)
vif(lm1)
