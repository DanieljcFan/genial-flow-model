...
revenue <- report_clean(petro$corp_name, petro$report_period,petro$营业总收入)
...
m <- Model(dat_petro$revenue,
           dat_petro[-1],
           vif=5,
           method = 'aic'
)

mean(abs(m$model$residuals/dat_petro$revenue))
summary(m$model)
vif(m$model)

df <- data.frame(time=t,
                 actual=dat_petro$revenue,
...
index <- m$index[-c(2,3,5)]
lm1 <- lm(revenue~., data = dat_petro[,c(0,index)+1])
mean(abs(lm1$residuals/dat_petro$revenue))
summary(lm1)
vif(lm1)
