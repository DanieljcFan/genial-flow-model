...
cost <- report_clean(glass$corp_name, glass$report_period, glass$营业总成本)
cost <- cost[order(cost$time),]
cost <- aggregate(cost$y, by=list(as.yearqtr(cost$time)), mean)
...
dat_glass <- merge(cost, X, by='time')
t <- dat_glass[,1]
...
m <- Model(dat_glass$cost,
           dat_glass[-1],
           vif = 10,
           method = 'aic'
)

mean(abs(m$model$residuals/dat_glass$cost))
summary(m$model)
vif(m$model)

df <- data.frame(time=t,
                 actual=dat_glass$cost,
                 pred=m$model$fitted.values)
df <- melt(df, id='time')
ggplot(df, aes(time, value, color=variable)) + geom_line()

index <- m$index[-c(2,3,4)]
lm1 <- lm(cost~., data = dat_glass[,c(0,index)+1])
mean(abs(lm1$residuals/dat_glass$cost))
summary(lm1)
vif(lm1)
