...
cash <- report_clean(glass$corp_name, glass$report_period, glass$经营活动现金净流量)
cash <- cash[order(cash$time),]
cash <- aggregate(cash$y, by=list(as.yearqtr(cash$time)), mean)
...
m <- Model(dat_glass$cash,
           dat_glass[-1],
           vif = 10,
           method = 'aic'
)

mean(abs(m$model$residuals/dat_glass$cash))
summary(m$model)
vif(m$model)

df <- data.frame(time=t,
                 actual=dat_glass$cash,
                 pred=m$model$fitted.values)
df <- melt(df, id='time')
ggplot(df, aes(time, value, color=variable)) + geom_line()

index <- m$index[-c(2,3,5,8)]
lm1 <- lm(cash~., data = dat_glass[,c(0,index)+1])
mean(abs(lm1$residuals/dat_glass$cash))
summary(lm1)
vif(lm1)
