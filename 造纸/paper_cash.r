...
cash <- report_clean(paper$corp_name, paper$report_period, paper$经营活动现金净流量)
cash[cash == 0] <- NA
...
mean(abs(m$model$residuals/dat_paper$cash))
summary(m$model)
vif(m$model)

df <- data.frame(time=t,
                 actual=dat_paper$cash,
