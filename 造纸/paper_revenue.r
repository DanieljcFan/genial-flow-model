...
revenue <- report_clean(paper$corp_name, paper$report_period, paper$营业总收入)
...
mean(abs(m$model$residuals/dat_paper$revenue))
summary(m$model)
vif(m$model)

df <- data.frame(time=t,
                 actual=dat_paper$revenue,
...
index <- m$index[-c(3,5)]
lm1 <- lm(revenue~., data = dat_paper[,c(0,index)+1])
mean(abs(lm1$residuals/dat_paper$revenue))
summary(lm1)
vif(lm1)
