...
cost <- report_clean(paper$corp_name, paper$report_period, paper$营业总成本)
cost[cost == 0] <- NA
...
# #各省产量加总
# index <- grep('产量.造纸', names(chain_season))
# chain_season$造纸产量 <- rowSums(chain_season[,index])
# chain_season <- chain_season[,-index]
# 
# #补充变量
# tmp <- read.csv('造纸成本.csv')
# names(tmp)[1] <- 'time'
...
#补充变量
tmp <- read.csv('造纸1.csv')
names(tmp)[1] <- 'time'
tmp$time <- as.POSIXlt(tmp$time, format='%Y-%m-%d')
tmp <- Ave(tmp)
index <- which(names(tmp) %in% names(chain_season))[-1]
chain_season <- merge(chain_season, tmp[-index], by='time')
...
X_all$season <- as.factor(sub('4|3','2',quarters(X_all$time)))

dat_paper <- merge(cost,X_all)
t <- dat_paper[,1]
dat_paper <- dat_paper[,-1]
...
mean(abs(m$model$residuals/dat_paper$cost))
summary(m$model)
vif(m$model)

df <- data.frame(time=t,
                 actual=dat_paper$cost,
                 pred=m$model$fitted.values)
df <- melt(df, id='time')
ggplot(df, aes(time, value, color=variable)) + geom_line()

# tmp <- dat_paper[,m$index+1]
# names(tmp) <- ''
# corr <- cor(tmp)
# corrplot.mixed(corr, diag = 'n', tl.pos = 'lt')

index <- m$index[-c(3,5,6)]
lm1 <- lm(cost~., data = dat_paper[,c(0,index)+1])
mean(abs(lm1$residuals/dat_paper$cost))
summary(lm1)
vif(lm1)
