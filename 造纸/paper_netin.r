...
netin <- report_clean(paper$corp_name, paper$report_period, paper$净利润)
netin[netin == 0] <- NA
...
mean(abs(m$model$residuals/dat_paper$netin))
summary(m$model)
vif(m$model)

df <- data.frame(time=t,
                 actual=dat_paper$netin,
...
index <- m$index[-c(8,9)]
lm1 <- lm(netin~., data = dat_paper[,c(0,index)+1])
mean(abs(lm1$residuals/dat_paper$netin))
summary(lm1)
vif(lm1)
