...
netin <- report_clean(wgoods$corp_name, wgoods$report_period, wgoods$净利润)
netin[netin == 0] <- NA
...
#产量加总
index <- grep('产量.*空调|产量.*冰箱|产量.*洗衣机', names(chain_season))
chain_season$白色家电产量 <- rowSums(chain_season[,index])

index <- grep('库存', names(chain_season))
chain_season$白色家电库存 <- rowSums(chain_season[,index])

index <- grep('出口数量', names(chain_season))
chain_season$白色家电出口 <- rowSums(chain_season[,index])

index <- grep('平均价格', names(chain_season))
chain_season$白色家电价格 <- rowMeans(chain_season[,index])
...
dat_wgoods <- merge(netin,X_all)
t <- dat_wgoods[,1]
dat_wgoods <- dat_wgoods[,-1]
...
m <- Model(dat_wgoods$netin,
           dat_wgoods[-1],
           vif=5,
           # must = ncol(dat_wgoods)-1,
           method = 'aic'
)

mean(abs(m$model$residuals/dat_wgoods$netin))
summary(m$model)
vif(m$model)

df <- data.frame(time=t,
                 actual=dat_wgoods$netin,
                 pred=m$model$fitted.values)
df <- melt(df, id='time')
ggplot(df, aes(time, value, color=variable)) + geom_line()

# tmp <- dat_wgoods[,m$index+1]
# names(tmp) <- ''
# corr <- cor(tmp)
# corrplot.mixed(corr, diag = 'n', tl.pos = 'lt')

index <- m$index[-c(3,6,7)]
lm1 <- lm(netin~., data = dat_wgoods[,c(0,index)+1])
mean(abs(lm1$residuals/dat_wgoods$netin))
summary(lm1)
vif(lm1)
...
# df <- melt(df, id='time')
# ggplot(df, aes(time, value, color=variable)) + geom_line()
# write.csv(df, file = '白色家电行业净利润.csv')
# sink('白色家电行业净利润模型.txt')
# summary(lm1)
# lm1$coefficients
# print('置信区间:')
# pred$fit
