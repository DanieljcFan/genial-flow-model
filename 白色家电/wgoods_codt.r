...
cost <- report_clean(wgoods$corp_name, wgoods$report_period, wgoods$营业总成本)
cost[cost == 0] <- NA
...
mean(abs(m$model$residuals/dat_wgoods$cost))
summary(m$model)
vif(m$model)

df <- data.frame(time=t,
                 actual=dat_wgoods$cost,
...
index <- m$index
lm1 <- lm(cost~., data = dat_wgoods[,c(0,index)+1])
mean(abs(lm1$residuals/dat_wgoods$cost))
summary(lm1)
vif(lm1)
...
# df <- melt(df, id='time')
# ggplot(df, aes(time, value, color=variable)) + geom_line()
# write.csv(df, file = '白色家电行业成本.csv')
# sink('白色家电行业成本模型.txt')
# summary(lm1)
# lm1$coefficients
# print('置信区间:')
