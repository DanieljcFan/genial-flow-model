...
cash <- report_clean(wgoods$corp_name, wgoods$report_period, wgoods$经营活动现金净流量)
cash[cash == 0] <- NA
...
mean(abs(m$model$residuals/dat_wgoods$cash))
summary(m$model)
vif(m$model)

df <- data.frame(time=t,
                 actual=dat_wgoods$cash,
...
lm1 <- lm(cash~., data = dat_wgoods[,c(0,index)+1])
mean(abs(lm1$residuals/dat_wgoods$cash))
summary(lm1)
vif(lm1)
...
# df <- melt(df, id='time')
# ggplot(df, aes(time, value, color=variable)) + geom_line()
# write.csv(df, file = '白色家电行业现金流.csv')
# sink('白色家电行业现金流模型.txt')
# summary(lm1)
# lm1$coefficients
# print('置信区间:')
# pred$fit
