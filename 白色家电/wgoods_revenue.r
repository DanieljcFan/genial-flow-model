...
revenue <- report_clean(wgoods$corp_name, wgoods$report_period, wgoods$营业总收入)
...
dat_wgoods <- merge(revenue,X_all)
t <- dat_wgoods[,1]
...
mean(abs(m$model$residuals/dat_wgoods$revenue))
summary(m$model)
vif(m$model)

df <- data.frame(time=t,
                 actual=dat_wgoods$revenue,
...
index <- m$index[-c(2,7)]
lm1 <- lm(revenue~., data = dat_wgoods[,c(0,index)+1])
mean(abs(lm1$residuals/dat_wgoods$revenue))
summary(lm1)
vif(lm1)
...
# df <- melt(df, id='time')
# ggplot(df, aes(time, value, color=variable)) + geom_line()
# write.csv(df, file = '白色家电行业营收.csv')
# sink('白色家电行业营收模型.txt')
# summary(lm1)
# lm1$coefficients
# print('置信区间:')
