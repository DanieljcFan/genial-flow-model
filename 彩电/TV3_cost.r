...
cost <- report_clean(TV$corp_name, TV$report_period, TV$营业总成本)
cost[cost == 0] <- NA
...
mean(abs(m$model$residuals/dat_TV$cost)) #相对误差
summary(m$model) 
vif(m$model)
...
#手动剔除变量，修改模型
index <- m$index[-c(2)]
lm1 <- lm(cost~., data = dat_TV[,c(0,index)+1])
mean(abs(lm1$residuals/dat_TV$cost))
summary(lm1)
vif(lm1)
