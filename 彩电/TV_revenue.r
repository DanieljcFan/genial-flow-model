...
revenue <- report_clean(TV$corp_name, TV$report_period, TV$营业总收入)
...
# #各省产量加总
# index <- grep('产量.视听', names(chain_season))
# chain_season$视听产量 <- rowSums(chain_season[,index])
# chain_season <- chain_season[,-index]
# 
# #补充变量
# tmp <- read.csv('视听营收.csv')
# names(tmp)[1] <- 'time'
...
mean(abs(m$model$residuals/dat_TV$revenue)) #相对误差
summary(m$model) 
vif(m$model)

df <- data.frame(time=t,
                 actual=dat_TV$revenue,
...
#相关系数图，辅助变量选择
# tmp <- dat_TV[,m$index+1]
# names(tmp) <- ''
# corr <- cor(tmp)
# corrplot.mixed(corr, diag = 'n', tl.pos = 'lt')

#手动剔除变量，修改模型
index <- m$index[-c(2,3,6)]
lm1 <- lm(revenue~., data = dat_TV[,c(0,index)+1])
mean(abs(lm1$residuals/dat_TV$revenue))
summary(lm1)
vif(lm1)
