...
netin <- report_clean(TV$corp_name, TV$report_period, TV$净利润)
netin[netin == 0] <- NA
...
mean(abs(m$model$residuals/dat_TV$netin)) #相对误差
summary(m$model) 
vif(m$model)
...
#手动剔除变量，修改模型
index <- m$index[-c(1,2,4,7)]
lm1 <- lm(netin~., data = dat_TV[,c(0,index)+1])
mean(abs(lm1$residuals/dat_TV$netin))
summary(lm1)
vif(lm1)
