...
cost <- report_clean(cement$corp_name, cement$report_period, cement$营业总成本)
cost[cost == 0] <- NA
...
dat_cement <- merge(cost, X, by='time')
t <- dat_cement[,1]
...
m <- Model(dat_cement$cost,
           dat_cement[-1],
           must=ncol(dat_cement)-1,
           method = 'aic'
)

mean(abs(m$model$residuals/dat_cement$cost))
summary(m$model)
vif(m$model)

df <- data.frame(time=t,
                 actual=dat_cement$cost,
...
index <- m$index[-c(6,7)]
lm1 <- lm(cost~., data = dat_cement[,c(0,index)+1])
mean(abs(lm1$residuals/dat_cement$cost))
summary(lm1)
vif(lm1)
