...
cash <- report_clean(copper$corp_name, copper$report_period, copper$经营活动现金净流量)
cash[cash == 0] <- NA
...
X_all <- merge(y_lag2,merge(y_lag3,X_all))
X_all$season <- as.factor(quarters(X_all$time))

dat_copper <- merge(cash, X_all, by='time')
t <- dat_copper[,1]
...
m <- Model(dat_copper$cash,
           dat_copper[-1],
           vif = 5,
           method = 'aic'
)

mean(abs(m$model$residuals/dat_copper$cash))
summary(m$model)
vif(m$model)
df <- data.frame(time=t,
                 actual=dat_copper$cash,
                 pred=m$model$fitted.values)
df <- melt(df, id='time')
ggplot(df, aes(time, value, color=variable)) + geom_line()


index <- m$index[-c(1,4,6,7,8)]
lm1 <- lm(cash~., data = dat_copper[,c(0,index)+1])
mean(abs(lm1$residuals/dat_copper$cash))
summary(lm1)
vif(lm1)
