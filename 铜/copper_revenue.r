...
revenue <- report_clean(copper$corp_name, copper$report_period, copper$营业总收入)
...
dat_copper <- merge(revenue, X, by='time')
t <- dat_copper[,1]
...
m <- Model(dat_copper$revenue,
           dat_copper[-1],
           vif = 10,
           method = 'aic'
)

mean(abs(m$model$residuals/dat_copper$revenue))
summary(m$model)
vif(m$model)
df <- data.frame(time=t,
                 actual=dat_copper$revenue,
                 pred=m$model$fitted.values)
df <- melt(df, id='time')
ggplot(df, aes(time, value, color=variable)) + geom_line()

index <- m$index[-c(2,3,5:7,9)]
lm1 <- lm(revenue~., data = dat_copper[,c(0,index)+1])
mean(abs(lm1$residuals/dat_copper$revenue))
summary(lm1)
vif(lm1)
