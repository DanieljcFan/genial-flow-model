...
index <- grep('饮料', report$申万二级)
drinking <- report[index,]
...
netin <- report_clean(drinking$corp_name, drinking$report_period, drinking$经营活动现金净流量)
netin[netin == 0] <- NA
...
#读入数据，需对Excel表做预处理
#注意时间列的格式（‘/’分割 or ‘-’分割）
setwd("I:/work/genial-flow/饮料/")
chain <- read.csv('饮料数据1.csv')
names(chain)[1] <- 'time'
...
# #各省产量加总
# index <- grep('产量.饮料', names(chain_season))
# chain_season$饮料产量 <- rowSums(chain_season[,index])
# chain_season <- chain_season[,-index]
 
# #补充变量
# tmp <- read.csv('饮料成本.csv')
# names(tmp)[1] <- 'time'
...
# X_all$season <- as.factor(sub('2|3', '1', quarters(X_all$time)))

dat_drinking <- merge(netin,X_all)
t <- dat_drinking[,1]
dat_drinking <- dat_drinking[,-1]
...
m <- Model(dat_drinking$netin,
           dat_drinking[-1],
           vif=10,
           # must = ncol(dat_drinking)-1,
           method = 'aic'
)

mean(abs(m$model$residuals/dat_drinking$netin))
summary(m$model)
vif(m$model)

df <- data.frame(time=t,
                 actual=dat_drinking$netin,
                 pred=m$model$fitted.values)
df <- melt(df, id='time')
ggplot(df, aes(time, value, color=variable)) + geom_line()

# tmp <- dat_drinking[,m$index+1]
# names(tmp) <- ''
# corr <- cor(tmp)
# corrplot.mixed(corr, diag = 'n', tl.pos = 'lt')

index <- m$index[-c(3)]
lm1 <- lm(netin~., data = dat_drinking[,c(0,index)+1])
mean(abs(lm1$residuals/dat_drinking$netin))
summary(lm1)
vif(lm1)
