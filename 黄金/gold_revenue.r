...
index <- grep('黄金', report$申万二级)
gold <- report[index,]

revenue <- report_clean(gold$corp_name, gold$report_period, gold$营业总收入)
...
# #各省产量加总
# index <- grep('产量.黄金', names(chain_season))
# chain_season$黄金产量 <- rowSums(chain_season[,index])
# chain_season <- chain_season[,-index]
# 
# #补充变量
# tmp <- read.csv('黄金成本.csv')
# names(tmp)[1] <- 'time'
...
#合并X,Y，时间戳另存为t
dat_gold <- merge(revenue,X_all)
t <- dat_gold[,1] 
dat_gold <- dat_gold[,-1]

#model#####
m <- Model(dat_gold$revenue,
           dat_gold[-1],
           vif=5,
           must = ncol(dat_gold)-1,
           method = 'aic')

mean(abs(m$model$residuals/dat_gold$revenue)) #相对误差
summary(m$model) 
vif(m$model)

df <- data.frame(time=t,
                 actual=dat_gold$revenue,
...
#手动剔除变量，修改模型
index <- m$index[-c(3,4)]
lm1 <- lm(revenue~., data = dat_gold[,c(0,index)+1])
mean(abs(lm1$residuals/dat_gold$revenue))
summary(lm1)
vif(lm1)
