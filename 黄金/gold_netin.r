...
netin <- report_clean(gold$corp_name, gold$report_period, gold$净利润)
netin[netin == 0] <- NA
...
#合并X,Y，时间戳另存为t
dat_gold <- merge(netin,X_all)
t <- dat_gold[,1] 
dat_gold <- dat_gold[,-1]

#model#####
m <- Model(dat_gold$netin,
           dat_gold[-1],
           vif=5,
           # must = ncol(dat_gold)-1,
           method = 'aic')

mean(abs(m$model$residuals/dat_gold$netin)) #相对误差
summary(m$model) 
vif(m$model)

df <- data.frame(time=t,
                 actual=dat_gold$netin,
...
#手动剔除变量，修改模型
index <- m$index[-c(3,4)]
lm1 <- lm(netin~., data = dat_gold[,c(0,index)+1])
mean(abs(lm1$residuals/dat_gold$netin))
summary(lm1)
vif(lm1)
