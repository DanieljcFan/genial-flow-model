...
netin <- report_clean(print$corp_name, print$report_period, print$净利润)
netin[netin == 0] <- NA
...
mean(abs(m$model$residuals/dat_print$netin))
summary(m$model)
vif(m$model)

df <- data.frame(time=t,
                 actual=dat_print$netin,
                 pred=m$model$fitted.values)
df <- melt(df, id='time')
ggplot(df, aes(time, value, color=variable)) + geom_line() +
  labs(title = '包装印刷行业净利润')
...
# ggplot(df, aes(time, value, color=variable)) + geom_line() +
#    labs(title = '包装印刷行业净利润')
# write.csv(df, file = '包装印刷行业净利润.csv')
# sink('包装印刷行业净利润模型.txt')
# summary(lm1)
# lm1$coefficients
# print('置信区间:')
# pred$fit
