...
revenue <- report_clean(whitewine$corp_name, whitewine$report_period, whitewine$营业总收入)
...
dat_whitewine <- merge(revenue,X_all)
t <- dat_whitewine[,1]
...
mean(abs(m$model$residuals/dat_whitewine$revenue))
summary(m$model)
vif(m$model)

df <- data.frame(time=t,
                 actual=dat_whitewine$revenue,
...
index <- m$index[-c(2,5,6,7)]
lm1 <- lm(revenue~., data = dat_whitewine[,c(0,index)+1])
mean(abs(lm1$residuals/dat_whitewine$revenue))
summary(lm1)
vif(lm1)
