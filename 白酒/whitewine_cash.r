...
cash <- report_clean(whitewine$corp_name, whitewine$report_period, whitewine$经营活动现金净流量)
cash[cash == 0] <- NA
...
X_all$season <- as.factor(sub('2|4','1',quarters(X_all$time)))

dat_whitewine <- merge(cash,X_all)
t <- dat_whitewine[,1]
...
mean(abs(m$model$residuals/dat_whitewine$cash))
summary(m$model)
vif(m$model)

df <- data.frame(time=t,
                 actual=dat_whitewine$cash,
...
index <- m$index[-c(2)]
lm1 <- lm(cash~., data = dat_whitewine[,c(0,index)+1])
mean(abs(lm1$residuals/dat_whitewine$cash))
summary(lm1)
vif(lm1)
