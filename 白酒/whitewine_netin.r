...
netin <- report_clean(whitewine$corp_name, whitewine$report_period, whitewine$净利润)
netin[netin == 0] <- NA
...
dat_whitewine <- merge(netin,X_all)
t <- dat_whitewine[,1]
...
mean(abs(m$model$residuals/dat_whitewine$netin))
summary(m$model)
vif(m$model)

df <- data.frame(time=t,
                 actual=dat_whitewine$netin,
...
index <- m$index[-c(4,5,7)]
lm1 <- lm(netin~., data = dat_whitewine[,c(0,index)+1])
mean(abs(lm1$residuals/dat_whitewine$netin))
summary(lm1)
vif(lm1)
