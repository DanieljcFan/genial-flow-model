...
revenue <- report_clean(plastic$corp_name, plastic$report_period, plastic$营业总收入)
...
#合并X,Y，时间戳另存为t
dat_plastic <- merge(revenue,X_all)
t <- dat_plastic[,1] 
dat_plastic <- dat_plastic[,-1]

#model#####
m <- Model(dat_plastic$revenue,
           dat_plastic[-1],
           vif=5,
           # must = ncol(dat_plastic)-1,
           method = 'aic')

mean(abs(m$model$residuals/dat_plastic$revenue)) #相对误差
summary(m$model) 
vif(m$model)

df <- data.frame(time=t,
                 actual=dat_plastic$revenue,
...
#手动剔除变量，修改模型
index <- m$index[-c(2,4)]
lm1 <- lm(revenue~., data = dat_plastic[,c(0,index)+1])
mean(abs(lm1$residuals/dat_plastic$revenue))
summary(lm1)
vif(lm1)
