...
netin <- report_clean(beer$corp_name, beer$report_period, beer$净利润)
netin[netin == 0] <- NA
...
mean(abs(m$model$residuals/dat_beer$netin))
summary(m$model)
vif(m$model)

df <- data.frame(time=t,
                 actual=dat_beer$netin,
...
index <- m$index[-c(3)]
lm1 <- lm(netin~., data = dat_beer[,c(0,index)+1])
mean(abs(lm1$residuals/dat_beer$netin))
summary(lm1)
vif(lm1)
