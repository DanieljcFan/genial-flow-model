...
cost <- report_clean(beer$corp_name, beer$report_period, beer$营业总成本)
cost[cost == 0] <- NA
...
mean(abs(m$model$residuals/dat_beer$cost))
summary(m$model)
vif(m$model)

df <- data.frame(time=t,
                 actual=dat_beer$cost,
...
index <- m$index
lm1 <- lm(cost~., data = dat_beer[,c(0,index)+1])
mean(abs(lm1$residuals/dat_beer$cost))
summary(lm1)
vif(lm1)
