...
cost <- report_clean(Aluminum$corp_name, Aluminum$report_period, Aluminum$营业总成本)
cost[cost == 0] <- NA
...
#合并X,Y，时间戳另存为t
dat_Aluminum <- merge(cost,X_all)
t <- dat_Aluminum[,1] 
dat_Aluminum <- dat_Aluminum[,-1]

#model#####
m <- Model(dat_Aluminum$cost,
           dat_Aluminum[-1],
           vif=5,
           must = c(180, 17, 122, 11),
           method = 'aic')

mean(abs(m$model$residuals/dat_Aluminum$cost)) #相对误差
summary(m$model) 
vif(m$model)

df <- data.frame(time=t,
                 actual=dat_Aluminum$cost,
...
#手动剔除变量，修改模型
index <- m$index[-c(5,9)]
lm1 <- lm(cost~., data = dat_Aluminum[,c(0,index)+1])
mean(abs(lm1$residuals/dat_Aluminum$cost))
summary(lm1)
vif(lm1)
