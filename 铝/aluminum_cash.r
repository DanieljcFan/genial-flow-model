...
cash <- report_clean(Aluminum$corp_name, Aluminum$report_period, Aluminum$经营活动现金净流量)
cash[cash == 0] <- NA
...
X_all$season <- as.factor(sub('3|4','2',quarters(X_all$time))) #sub可将不同季度合并，减少变量数

#合并X,Y，时间戳另存为t
dat_Aluminum <- merge(cash,X_all)
t <- dat_Aluminum[,1] 
dat_Aluminum <- dat_Aluminum[,-1]

#model#####
m <- Model(dat_Aluminum$cash,
           dat_Aluminum[-1],
           vif=5,
           must = c(105, 151, 71, 9, 62),
           method = 'aic')

mean(abs(m$model$residuals/dat_Aluminum$cash)) #相对误差
summary(m$model) 
vif(m$model)

df <- data.frame(time=t,
                 actual=dat_Aluminum$cash,
...
#手动剔除变量，修改模型
index <- m$index[-c(7,9,12,13)]
lm1 <- lm(cash~., data = dat_Aluminum[,c(0,index)+1])
mean(abs(lm1$residuals/dat_Aluminum$cash))
summary(lm1)
vif(lm1)
