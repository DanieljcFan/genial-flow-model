...
revenue <- report_clean(Aluminum$corp_name, Aluminum$report_period, Aluminum$营业总收入)
...
#合并X,Y，时间戳另存为t
dat_Aluminum <- merge(revenue,X_all)
t <- dat_Aluminum[,1] 
dat_Aluminum <- dat_Aluminum[,-1]

#model#####
m <- Model(dat_Aluminum$revenue,
           dat_Aluminum[-1],
           vif=5,
           must = ncol(dat_Aluminum)-1,
           method = 'aic')

mean(abs(m$model$residuals/dat_Aluminum$revenue)) #相对误差
summary(m$model) 
vif(m$model)

df <- data.frame(time=t,
                 actual=dat_Aluminum$revenue,
...
#手动剔除变量，修改模型
index <- m$index[-c(3,6,8,9)]
lm1 <- lm(revenue~., data = dat_Aluminum[,c(0,index)+1])
mean(abs(lm1$residuals/dat_Aluminum$revenue))
summary(lm1)
vif(lm1)
