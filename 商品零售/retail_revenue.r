...
revenue <- report_clean(retail$corp_name, retail$report_period, retail$营业总收入)
...
#合并X,Y，时间戳另存为t
dat_retail <- merge(revenue,X_all)
t <- dat_retail[,1] 
...
mean(abs(m$model$residuals/dat_retail$revenue)) #相对误差
summary(m$model) 
vif(m$model)

df <- data.frame(time=t,
                 actual=dat_retail$revenue,
...
#手动剔除变量，修改模型
index <- m$index[-c(3,5,8)]
lm1 <- lm(revenue~., data = dat_retail[,c(0,index)+1])
mean(abs(lm1$residuals/dat_retail$revenue))
summary(lm1)
vif(lm1)
...
# ggplot(df, aes(time, value, color=variable)) + geom_line() +
#    labs(title = '商品零售行业营收')
# write.csv(df, file = '商品零售行业营收.csv')
# sink('商品零售行业营收模型.txt')
# summary(lm1)
# lm1$coefficients
# print('置信区间:')
