...
netin <- report_clean(retail$corp_name, retail$report_period, retail$净利润)
netin[netin == 0] <- NA
...
mean(abs(m$model$residuals/dat_retail$netin)) #相对误差
summary(m$model) 
vif(m$model)

df <- data.frame(time=t,
                 actual=dat_retail$netin,
                 pred=m$model$fitted.values)
df <- melt(df, id='time')
ggplot(df, aes(time, value, color=variable)) + geom_line() +
  labs(title = '商品零售行业净利润')
...
lm1 <- lm(netin~., data = dat_retail[,c(0,index)+1])
mean(abs(lm1$residuals/dat_retail$netin))
summary(lm1)
vif(lm1)
...
# ggplot(df, aes(time, value, color=variable)) + geom_line() +
#    labs(title = '商品零售行业净利润')
# write.csv(df, file = '商品零售行业净利润.csv')
# sink('商品零售行业净利润模型.txt')
# summary(lm1)
# lm1$coefficients
# print('置信区间:')
# pred$fit
